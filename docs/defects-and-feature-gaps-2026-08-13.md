...
- [x] 独立热榜页、视频/GIF 动态、投票帖、置顶评论、编辑历史 —— **决策:不需要做**（保留记录，不实现）。
...
- [x] 资料完整度无自我引导 → 已加 `ProfileCompletenessService`(0-10 分 + 缺失项),`GET /users/me/completeness`,`我的`页「还缺 xxx」引导条。
- [x] 破冰话题未嵌入会话开场 —— **决策:不需要做**。
- [x] 拉黑后残留数据 → 已清理:拉黑即删双向关注(user_follow)+ 双向心动(user_like)+ 配对(user_match)。
- [x] 见面安全提醒 → 配对成功通知时追加安全提醒(公共场所/告知亲友)。`VipRecord` 死表 —— **决策:不需要做**。
