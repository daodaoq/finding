...
- [ ] 建群群名先赋值后清洗（`GroupChatService.java:51`）：落库未清洗原文（存储型 XSS）；评论清洗后未判空（`PostServiceImpl.java:297`）。
