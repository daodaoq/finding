...
interface Guide { id: number; title: string; subtitle: string; content: string; category: string; userId: number; createdAt: string; }
...
export default function LoveGuideReview() {
  ...
  const columns: ColumnsType<Guide> = [
    ...
    { title: '操作', width: 175, render: (_, row) => <Space><a onClick={() => setDetail(row)}>查看</a><a style={{ color: token.colorSuccess }} onClick={() => review(row, true)}>通过</a><a style={{ color: token.colorError }} onClick={() => setReject(row)}>拒绝</a></Space> },
  ];
