...
public class SensitiveWordFilter {
    ...
    private static final String CACHE_KEY = "forbidden:words";
    ...
    private void refresh() {
        List<String> words = redisUtils.get(CACHE_KEY);
        if (words == null) {
            WordProvider provider = wordProvider.getIfAvailable();
            words = provider != null ? provider.enabledWords() : Collections.emptyList();
            redisUtils.set(CACHE_KEY, words, CACHE_TTL_HOURS, TimeUnit.HOURS);
        }
